...
const getChatConDestinatario = async (req, res, next) => {
  try {
    const usuario = await getInternalUser(req.user.auth_user_id);
    if (!usuario) return res.status(404).json({ error: "Perfil no encontrado" });
...
const enviarMensaje = async (req, res, next) => {
  try {
    ...
    if (!hasAccess) return res.status(403).json({ error: "No tienes permiso para comunicarte sobre este alumno." });
    ...
    if (usuario.rol_id === ROLES.DOCENTE) {
      ...
      if (!padresIds.length) return res.status(400).json({ error: "El alumno no tiene padres" });
      ...
      if (!docente_id) return res.status(400).json({ error: "docente_id es requerido para padres" });
...
const getPadresDeAlumno = async (req, res, next) => {
  try {
    ...
    if (!hasAccess) return res.status(403).json({ error: "No tienes permiso para ver los padres de este alumno." });
...
const getChatConPadre = async (req, res, next) => {
  try {
...
const getTodosChatsDeAlumno = async (req, res, next) => {
  try {
    const alumnoId = req.params.alumno_id;
    const usuario = await getInternalUser(req.user.auth_user_id);
    if (!usuario) return res.status(404).json({ error: "Perfil no encontrado" });
    ...
    const hasAccess = await verifyAccessToStudent(usuario, alumnoId);
    if (!hasAccess) return res.status(403).json({ error: "No tienes permiso para ver los chats de este alumno." });
    ...
    allMessages.sort((a, b) => new Date(a.creado_en) - new Date(b.creado_en));
...
const getDocentesDeAlumno = async (req, res, next) => {
  try {
    const alumnoId = req.params.alumno_id;
    const usuario = await getInternalUser(req.user.auth_user_id);
    if (!usuario) return res.status(404).json({ error: "Perfil no encontrado" });
    ...
    const hasAccess = await verifyAccessToStudent(usuario, alumnoId);
    if (!hasAccess) return res.status(403).json({ error: "No tienes permiso para ver los docentes de este alumno." });
    ...
    if (alumnoErr || !alumno) return res.status(404).json({ error: "Alumno no encontrado" });
...
const getChatAlumnoDocente = async (req, res, next) => {
  try {
    const alumnoId = req.params.alumno_id;
    const usuario = await getInternalUser(req.user.auth_user_id);
    if (!usuario) return res.status(404).json({ error: "Perfil no encontrado" });
    ...
    const hasAccess = await verifyAccessToStudent(usuario, alumnoId);
    if (!hasAccess) return res.status(403).json({ error: "No tienes permiso para ver este chat." });
...
const getChatIndividualDeAlumno = async (req, res, next) => {
  try {
...
module.exports = {
  getContactos,
  getChatConDestinatario,
  enviarMensaje,
  getPadresDeAlumno,
  getChatConPadre,
  getTodosChatsDeAlumno,
  getDocentesDeAlumno,
  getChatAlumnoDocente,
  getChatIndividualDeAlumno,
  // Para testing:
  getInternalUser,
  verifyAccessToStudent
};
