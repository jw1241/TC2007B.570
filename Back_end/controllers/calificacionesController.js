...
const schema = Joi.object({
  alumnoId: Joi.string().uuid().required()
});
  ...
  try {
    ...
    if (error) {
      return res.status(400).json({
        error: { message: "Alumno inválido" }
      });
    }

    const { alumnoId } = value;

    console.log("VALIDATED ALUMNO ID =", alumnoId);
    ...
    console.log("PARENT CHECK:", req.user.id, alumnoId);
    ...
    if (req.user.rol_id === ROLES.DOCENTE) {
      ...
      materiaIds = asignaciones.map(a => a.materia_id);
    }
    ...
    console.log("PARENT QUERY RESULT", parentescos);

    const padres = (parentescos || [])
      .map(p => p.usuarios)
      .filter(Boolean);

    if (parentescoLookupError) throw parentescoLookupError;

    /**
     * LOAD GRADES
     */
    let gradesQuery = req.supabase
      .from("calificaciones")
      .select(`
        id,
        nota,
    ...
    if (req.user.rol_id === ROLES.DOCENTE && materiaIds.length > 0) {
      gradesQuery = gradesQuery.in("materia_id", materiaIds);
    }
    ...
    const materias = Object.values(grouped); 
    ...
    if (calificaciones?.length > 0 && materias.length > 0) {
      const total = calificaciones.reduce((sum, item) => sum + Number(item.nota), 0);
      promedio = Number((total / calificaciones.length).toFixed(1));

      const subjectAverages = materias.map(m => {
        const avg = m.calificaciones.reduce((s, c) => s + Number(c.nota), 0) / m.calificaciones.length;
        return {
      ...
      mejorMateria = subjectAverages.reduce((best, curr) => curr.promedio > best.promedio ? curr : best);
      peorMateria = subjectAverages.reduce((worst, curr) => curr.promedio < worst.promedio ? curr : worst);
    }
